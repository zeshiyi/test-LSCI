...
class CLIP(nn.Module):
    ...
    def __init__(
        ...
        # self.queue_size = 57856
        self.logit_scale = nn.Parameter(torch.ones([]) * init_logit_scale)
        if init_logit_bias is not None:
            self.logit_bias = nn.Parameter(torch.ones([]) * init_logit_bias)
        else:                                                                                                                                                                                                                                                                                                     
            self.logit_bias = nn.Parameter(torch.ones([]) * 0.0)
    ...
    def state_dict(self, destination=None, prefix='', keep_vars=False):
        ...
        for no_save_key in self._no_save_params:
            keys_to_remove = [k for k in state_dict.keys() if no_save_key in k]
    ...
    def forward(
        ...
        bs = image.size(0)
        with torch.no_grad():
            weights_i2t = F.softmax(sim_i2t_m[:,:bs], dim=1) + 1e-8
            weights_i2t.fill_diagonal_(0)
            weights_t2i = F.softmax(sim_t2i_m[:,:bs], dim=1) + 1e-8
